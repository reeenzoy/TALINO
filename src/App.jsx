...
export default function App() {
  ...
  const [messages, setMessages] = useState([]); // {role, content, loading?, complete?}
  ...
  const streamBotMessage = (fullMsg, botMsgIndex, onDone) => {
    // ensure the placeholder bot message exists & is not loading text
    setMessages((prev) => {
      const arr = [...prev];
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axios.post('/api/chat', { input: messageToSend, session_id: sessionId }, { signal: controller.signal });
        ...
        <div className="flex">
          ...
          <div className="min-w-0 flex-1">
            ...
            <header className="sticky top-0 z-10 border-b border-borderc bg-bgp/60 backdrop-blur">
              <div className="mx-auto flex h-14 w-full max-w-5xl items-center justify-between px-4">
                <div className="flex items-center gap-3">
                  {/* <button
                    className="rounded-lg border border-borderc px-3 py-1 text-sm text-texts hover:bg-bgs"
                    onClick={newSession}
                  >
                    + New Session
                  </button> */}
            ...
            <section className="mx-auto grid w-full max-w-5xl grid-rows-[1fr_auto] gap-4 px-4 pb-6 pt-6">
              ...
              {/* Composer pinned at bottom on chat screens */}
              {messages.length > 0 && (
                <div className="mx-auto w-full max-w-3xl">
                  <Composer
                    value={input}
